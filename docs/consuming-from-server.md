...
业务仓库固定精确 `0.8.0`，并按 [升级到 0.8.0](upgrading-to-0.8.0.md) 使用空库。验证未发布提交时，维护者创建唯一本地测试版本：
...
ZYBLW_AGENT_VERSION=0.8.0-local sbt -batch test
```

业务 build 只有在显式本地开关下才添加 `Resolver.mavenLocal`。生产和普通 CI 不启用该 resolver，避免开发机
`~/.m2` 中的同名内容覆盖可信 Central 制品。任何 `0.8.0-local` 都不得上传 Maven Central。
