...
在 Maven Central 首次正式版本发布前，可以在框架目录执行
`sbt -batch 'set ThisBuild / version := "0.1.0-local"; publishM2'`，宿主临时使用 `0.1.0-local` 并显式启用 Maven
Local；不要把本地版本或 `SNAPSHOT` 当作可重复生产发布物。完整命令见
