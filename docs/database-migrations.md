...
兼容模式必须显式配置两个 location，并继续使用原 history：
...
这是已有数据的迁移桥，不是新应用模板。未来若要拆分 history，需要专门的 adoption migration/运维方案，先核对表、约束、
索引和 checksum，再写入新的历史；禁止仅使用 `baselineOnMigrate` 隐藏未知 schema。
