# PostgreSQL 自动迁移、结构校验与 0.4 知识库基线

> 状态：0.4 开发主线；0.3 已发布 migration 保持冻结
> 最后核验：2026-08-02
...
for
  ...
  _ <- AgentPostgresMigrations.migrateKnowledge1536(dataSource) // 需要 RAG 时
yield ()
```
...
val rag  = PostgresAgentPersistence.migratedKnowledge1536()
```
...
核心 history table 位于宿主当前 schema，名为 `flyway_zyblw_agent_schema_history`。0.4 的 1536 维知识库使用独立 location，固定管理
`zyblw_agent_knowledge` schema，并把独立 `flyway_zyblw_agent_knowledge_1536_history` 放在其中；实际 location 只有一份
`V001__agent_knowledge_pgvector_1536_baseline.sql`，已一次性包含 manifest、staging、active chunks、FTS、HNSW、parent/neighbor、
...
`migrateKnowledge1536` 或 `migrateCoreAndKnowledge1536`。
...
- staging/active 两张表的 embedding 是否真实为 `vector(1536)`。
...
创建共享 DataSource
  ...
  -> knowledge-1536 schema migrate/validate/verify（按需）
  -> 业务 migration
...
核心 V001 和旧 `optional/pgvector` V001 从 `0.3.0` 发布提交起永久冻结。虽然当前业务尚未上线，公开 Maven 制品已经可能被第三方执行；直接合并
V002 回旧 V001 会制造 checksum 漂移。0.4 因此使用新的 pgvector location 单文件基线，未发布的 V002 已删除并合入该基线。任何已发布 checksum
