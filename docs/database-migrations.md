...
核心 location 包含冻结的 `V001__zyblw_agent_0_3_baseline.sql` 和每次覆盖写入的
`R__zyblw_agent_schema_comments.sql`。中文数据字典必须覆盖全部 26 张控制面表及其字段；未映射名称会让
repeatable migration 失败。框架不会因为 JAR 或 `DataSource` 出现在 classpath 就修改数据库；宿主需要在受控启动阶段选择
下面一种模式。
...
- 核心 26 张关键表是否存在于当前 schema，避免临时表或其他 schema 的同名对象蒙混通过；
...
- 核心 26 张表的全部字段是否同样拥有专属中文说明（由 `R__zyblw_agent_schema_comments.sql` 每次覆盖写入）。
