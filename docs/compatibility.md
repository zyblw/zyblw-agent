...
`0.3.0` 明确破坏 `0.2.x` 的 Scala API、二进制、持久化 outcome 和数据库历史。原因不是“版本早期就可以随意变化”，
而是发布前选择用一次清晰重建换取正确的 durable command 与 Workflow wait/wakeup 数据模型。已经发布的 `0.1.0`、
`0.2.0`、`0.2.1` Maven 制品、Git 标签和 migration 保持客观不可变。

本次破坏窗口结束于 `0.3.0`：后续 `0.3.x` patch 必须遵守 minor 内兼容承诺；下一次有意破坏必须提升 minor。

## 0.3.0 发布边界

| 表面 | 0.3.0 基线 | 后续 0.3.x patch 承诺 |
|---|---|---|
| 公共 Scala API | 相对 0.2.x 已允许删除、重命名和重构；本表冻结发布时的 0.3.0 公共 API | 不删除或改变公开签名语义 |
...
详细步骤见[升级到 0.3.0](upgrading-to-0.3.0.md)。
