-- zyblw-agent 框架基础表（全新数据库手工初始化版）
-- 正式发布优先使用 Flyway；本文件不得写入真实密码。
...
CREATE INDEX IF NOT EXISTS agent_runs_session_updated_idx ON agent_runs(session_id, updated_at DESC);
CREATE INDEX IF NOT EXISTS agent_runs_status_updated_idx ON agent_runs(status, updated_at DESC);
CREATE UNIQUE INDEX IF NOT EXISTS agent_runs_start_idempotency_idx
...
CREATE UNIQUE INDEX IF NOT EXISTS tool_executions_batch_ordinal_idx
  ON tool_executions(run_id, batch_id, ordinal);
CREATE INDEX IF NOT EXISTS tool_executions_batch_status_idx
  ON tool_executions(run_id, batch_id, status, ordinal);
...
CREATE INDEX IF NOT EXISTS approval_requests_run_status_idx ON approval_requests(run_id, status);
...
CREATE INDEX IF NOT EXISTS agent_memories_scope_active_idx
  ON agent_memories(scope_kind, scope_key, updated_at DESC, memory_key ASC) WHERE status = 'active';
CREATE INDEX IF NOT EXISTS agent_memories_search_idx
  ON agent_memories USING GIN(search_vector) WHERE status = 'active';
CREATE INDEX IF NOT EXISTS agent_memories_expiry_idx
  ON agent_memories(expires_at, scope_kind, scope_key, memory_key)
  WHERE status = 'active' AND expires_at IS NOT NULL;
CREATE INDEX IF NOT EXISTS agent_memories_user_governance_idx
  ON agent_memories(tenant_id, user_id, updated_at DESC) WHERE scope_kind = 'user';

-- 用户记忆治理的低敏不可变审计；不保存正文、搜索词、原始 memory_key、scopes 或 attributes。
...
CREATE INDEX IF NOT EXISTS agent_memory_audit_actor_idx
  ON agent_memory_audit(actor_tenant_id, actor_user_id, occurred_at DESC, audit_id);
CREATE INDEX IF NOT EXISTS agent_memory_audit_target_idx
  ON agent_memory_audit(target_scope_kind, target_scope_key, occurred_at DESC, audit_id);
CREATE INDEX IF NOT EXISTS agent_memory_audit_action_idx
...
CREATE INDEX IF NOT EXISTS agent_run_commands_claimable_idx
  ON agent_run_commands(priority DESC, available_at ASC, created_at ASC, command_id ASC)
  WHERE status = 'Queued';
CREATE INDEX IF NOT EXISTS agent_run_commands_run_created_idx
  ON agent_run_commands(run_id, created_at ASC, command_id ASC);

CREATE TABLE IF NOT EXISTS agent_run_dispatch (
  ...
  CHECK (
    ...
    OR (status <> 'Leased' AND current_command_id IS NULL AND lease_owner IS NULL AND lease_token IS NULL
      AND claimed_at IS NULL AND lease_expires_at IS NULL)
  )
);
CREATE INDEX IF NOT EXISTS agent_run_dispatch_owner_idx
  ON agent_run_dispatch(lease_owner, lease_expires_at)
  WHERE status = 'Leased';

-- producer 业务幂等记录。Executing 和 mutation/outbox 同事务，因此不会独立提交为半成品。
CREATE TABLE IF NOT EXISTS agent_business_operations (
...
CREATE INDEX IF NOT EXISTS agent_business_operations_run_idx
  ON agent_business_operations(run_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS agent_outbox_operation_idx ON agent_outbox_events(operation_id, ordinal);
CREATE INDEX IF NOT EXISTS agent_outbox_lease_idx
  ON agent_outbox_events(lease_owner, lease_expires_at) WHERE status = 'Publishing';

-- consumer 去重与其业务 mutation 必须共用一次提交。
...
CREATE INDEX IF NOT EXISTS agent_inbox_received_idx
  ON agent_inbox_messages(consumer_name, received_at DESC);
...
CREATE INDEX IF NOT EXISTS agent_compensations_pending_idx
  ON agent_compensations(available_at ASC, created_at ASC, compensation_id ASC)
  WHERE status = 'Pending';
CREATE INDEX IF NOT EXISTS agent_compensations_lease_idx
...
CREATE INDEX IF NOT EXISTS agent_embedding_cache_expiry_idx
...
CREATE TABLE IF NOT EXISTS agent_embedding_quota_windows (
  tenant_id TEXT NOT NULL CHECK (length(trim(tenant_id)) BETWEEN 1 AND 1000),
...
CREATE INDEX IF NOT EXISTS agent_embedding_quota_windows_cleanup_idx
...
CREATE TABLE IF NOT EXISTS agent_embedding_quota_reservations (
  tenant_id TEXT NOT NULL CHECK (length(trim(tenant_id)) BETWEEN 1 AND 1000),
...
CREATE INDEX IF NOT EXISTS agent_embedding_quota_reservations_window_idx
...
CREATE TABLE IF NOT EXISTS agent_eval_snapshots (
  ...
  -- PostgreSQL TIMESTAMPTZ 为微秒精度；额外保存秒/nano 才能与 JVM Instant 形成完全确定的排序。
  ...
  -- TEXT 保留应用生成的确定性字节用于 checksum；JSONB 提供查询能力，但会规范化原始文本。
...
-- 声明式 Workflow 的完整恢复快照；Workflow 可独立于模型 Agent Run，因此不引用 agent_runs。
...
CREATE INDEX IF NOT EXISTS agent_workflow_checkpoints_identity_idx
...
CREATE INDEX IF NOT EXISTS agent_workflow_node_executions_claim_idx
  ON agent_workflow_node_executions(status, lease_expires_at, updated_at)
  WHERE status IN ('Running', 'Prepared');

CREATE INDEX IF NOT EXISTS agent_workflow_node_executions_identity_idx
