...
之后、打分之前。默认切分按 `maxTokens=512`。`text-embedding-3` 设 `EMBEDDING_TOKENIZER=cl100k-base`；通义 / qwen3 文本向量设 `cjk-approx-v1`；OpenAI 4o 系列设 `o200k-base`。live 摄入缺这个值会在装配期失败。模型侧使用 `knowledge_search` / `knowledge_fetch`，不得覆盖
