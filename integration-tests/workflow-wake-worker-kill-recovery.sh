...
probe_class=com.zyblw.agent.examples.WorkflowWakeWorkerProcessKillProbe
container_name="zyblw-agent-workflow-kill-$$"
work_dir=$(mktemp -d "${TMPDIR:-/tmp}/zyblw-agent-workflow-kill.XXXXXX")
...
docker run --detach --name "$container_name" \
  ...
  --env POSTGRES_PASSWORD=zyblw-workflow-kill-test \
...
db_user=zyblw
db_password=zyblw-workflow-kill-test
...
echo "[1/4] seed and signal a durable Workflow wait"
run_probe seed

echo "[2/4] start old Workflow Worker and wait for both generation-1 leases"
...
for _ in $(seq 1 240); do
  if [[ -s "$old_report" ]] \
    && awk -F= '$1 == "wakeGeneration" && $2 == "1" { wake = 1 } $1 == "executionGeneration" && $2 == "1" { execution = 1 } END { exit !(wake && execution) }' "$old_report"; then
    reported=true
    break
  fi
  if ! kill -0 "$old_sbt_pid" 2>/dev/null; then
    echo "old Workflow Worker launcher exited before reporting both leases" >&2
...
if [[ "$reported" != true ]]; then
  echo "old Workflow Worker did not report both leases" >&2
  sed -n '1,240p' "$old_log" >&2
  exit 1
fi
...
if [[ ! "$old_worker_pid" =~ ^[1-9][0-9]*$ ]] || ! kill -0 "$old_worker_pid" 2>/dev/null; then
  echo "reported old Workflow Worker PID is not alive: $old_worker_pid" >&2
  exit 1
fi

echo "      SIGKILL old Workflow Worker JVM pid=$old_worker_pid"
kill -KILL "$old_worker_pid"
old_worker_pid=
if wait "$old_sbt_pid"; then
  echo "old Workflow Worker launcher unexpectedly succeeded after SIGKILL" >&2
  exit 1
fi
old_sbt_pid=
...
echo "[3/4] reclaim wake and execution leases in a new Workflow Worker JVM"
run_probe recover

echo "[4/4] verify consumed wait, terminal checkpoint and both generation transitions"
run_probe verify

echo "PASS: an OS-killed Workflow wake Worker was reclaimed through the production PostgreSQL workflow protocol"
