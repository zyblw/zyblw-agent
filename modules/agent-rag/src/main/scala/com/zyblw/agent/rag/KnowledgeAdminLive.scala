...
object KnowledgeAdminLive:
  ...
  def documentView(manifest: KnowledgeIndexManifest): KnowledgeDocumentView = KnowledgeDocumentView(
    ...
    embeddingProvider = manifest.build.buildSpec.embeddingProvider,
    embeddingModel = manifest.build.buildSpec.embeddingModel,
    embeddingDimension = manifest.build.buildSpec.embeddingDimension,
