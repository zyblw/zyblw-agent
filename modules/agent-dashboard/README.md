...
| 知识库 | 会话租户索引清单（游标翻页）、检索沙盒 | `knowledge` |
...
| `ragTenant` | 检索沙盒的模拟租户；清单始终使用会话租户 |
...
| `agent:admin:write` | 改变部署行为：工具白名单、审批策略、模型切换、死信重排（蕴含读权限） |
| `agent:admin:debug` | 产生真实 Provider 费用：检索沙盒、模型探活（不被写权限蕴含） |
| `knowledge:read` | 知识清单与检索沙盒（沙盒另外需要 `agent:admin:debug`） |
| `knowledge:write` | 退役当前 Active 索引版本 |
...
- **费用可见**。检索沙盒和模型探活会调用 Provider 并产生真实费用，因此它们只在点击时发起请求，
  不因窗口重新聚焦或组件重挂载而自动重发，按钮上也如实标注这一点。控制台不摄入问答知识。
- **每项配置只有一个写入口**。同一项配置若存在两个可写界面，两处的校验逻辑会逐渐分叉。因此工具治理只在
  配置页可改，模型工作点只在模型页可改，安全页始终只读。
- **写操作必有回执**。配置保存、死信重排、索引退役、模型切换与探活都会给出一条通知；这些操作
