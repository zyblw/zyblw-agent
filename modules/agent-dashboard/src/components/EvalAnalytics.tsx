      ...
      <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 flex flex-wrap items-center justify-between gap-4 shadow-xl backdrop-blur-sm">
          ...
          <div>
            <div className="flex items-center space-x-2">
              <h2 className="font-bold text-base text-slate-100">
                Fail-Closed Release Gate & Quality Analytics
              </h2>
              <span className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 text-xs px-2.5 py-0.5 rounded-full font-bold flex items-center gap-1">
                ...
                Gate Passed ({latestTrend.commitHash})
              </span>
            </div>
            <p className="text-xs text-slate-400 mt-0.5">
              Repeated Experiment Grader • pass@k / pass^k Metrics • Multi-Trial CI Trend Pipeline
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-4 text-xs font-mono">
          <div className="bg-slate-950 px-3 py-1.5 rounded-lg border border-slate-800">
            <span className="text-slate-500 text-[10px] block">pass@k (AT-LEAST-ONCE)</span>
          ...
          <div className="bg-slate-950 px-3 py-1.5 rounded-lg border border-slate-800">
            <span className="text-slate-500 text-[10px] block">pass^k (ALL-SUCCESS)</span>
      ...
      <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 shadow-xl backdrop-blur-sm">
        <div className="pb-4 border-b border-slate-800 mb-4 flex items-center justify-between">
          <h3 className="font-bold text-sm text-slate-200 flex items-center gap-2">
            ...
            Historical Quality Metric Trends across Releases
          </h3>
          <span className="text-xs text-slate-500 font-mono">AgentEvalRunner Snapshots</span>
        </div>
            ...
            >
              ...
              <div className="space-y-2 text-xs font-mono">
                <div className="flex justify-between items-center">
                  <span className="text-slate-400">pass@k Score:</span>
                ...
                <div className="flex justify-between items-center pt-2 border-t border-slate-900 text-[11px]">
                  <span className="text-slate-500">Citation Correctness:</span>
                  <span className="text-cyan-300">{(item.citationAccuracy * 100).toFixed(1)}%</span>
                </div>

                <div className="flex justify-between items-center text-[11px]">
                  <span className="text-slate-500">Tool Selection Accuracy:</span>
      ...
      <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 shadow-xl backdrop-blur-sm">
        <div className="pb-4 border-b border-slate-800 mb-4 flex items-center justify-between">
          <h3 className="font-bold text-sm text-slate-200 flex items-center gap-2">
            ...
            Token Telemetry & Model Cost Breakdowns
          </h3>
          <span className="text-xs text-slate-500 font-mono">OpenTelemetry GenAI Conventions</span>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
            <span className="text-slate-500 text-[10px] font-mono block">TOTAL TOKEN USAGE</span>
            <span className="text-xl font-bold text-slate-100 font-mono">1,280,400</span>
            <span className="text-[10px] text-slate-400 mt-1 block">Across all provider calls</span>
          </div>

          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
            <span className="text-slate-500 text-[10px] font-mono block">PROMPT CACHE HITS</span>
            <span className="text-xl font-bold text-emerald-400 font-mono">68.4%</span>
            <span className="text-[10px] text-slate-400 mt-1 block">Saved input token costs</span>
          </div>

          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
            <span className="text-slate-500 text-[10px] font-mono block">REASONING TOKENS</span>
            <span className="text-xl font-bold text-cyan-400 font-mono">14.2%</span>
            <span className="text-[10px] text-slate-400 mt-1 block">DeepSeek/Gemini thinking</span>
          </div>

          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
            <span className="text-slate-500 text-[10px] font-mono block">ESTIMATED RUN COST</span>
            <span className="text-xl font-bold text-amber-400 font-mono">$1.42 USD</span>
            <span className="text-[10px] text-slate-400 mt-1 block">Configured rate estimate</span>
          </div>
        </div>
      </div>
    </div>
