-- 本 migration 是可选能力，不放入默认 Flyway 路径。
-- 1536 必须与业务选择的 Embedding Provider 维度一致；若维度不同，请复制并修改 vector(1536)。
CREATE EXTENSION IF NOT EXISTS vector;

-- 每次重新切分或更换 Embedding 模型都创建新版本。只有一个 Ready 版本可以 active。
...
CREATE INDEX IF NOT EXISTS agent_knowledge_documents_recovery_idx
  ON agent_knowledge_documents(status, updated_at)
...
-- 暂存表对 worker 重放执行 upsert；只有 activate 的短事务才把完整版本发布到正式 chunks 表。
...
CREATE TABLE IF NOT EXISTS agent_knowledge_chunks (
  ...
  chunk_text TEXT NOT NULL,
  -- search_text 允许业务在摄取阶段写入中文分词结果；不提供时 Adapter 写原文。
  search_text TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS agent_knowledge_chunks_permissions_idx
  ON agent_knowledge_chunks USING GIN(permissions);

CREATE INDEX IF NOT EXISTS agent_knowledge_chunks_metadata_idx
  ON agent_knowledge_chunks USING GIN(metadata jsonb_path_ops);

CREATE INDEX IF NOT EXISTS agent_knowledge_chunks_search_vector_idx
  ON agent_knowledge_chunks USING GIN(search_vector);

-- HNSW 适合查询多、持续写入的知识库；上线前需按数据量和召回评测调整 m/ef_construction。
